...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect total_bb(pos, pos + ImVec2(label_size.x + square_sz + 5, 20));
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y), label, 0, true);
    //window->DrawList->AddText(check_bb.Min + ImVec2(pad, pad) + ImVec2(18, 0), *v ? ImColor(250, 250, 250, 220) : ImColor(170, 170, 170, 205), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, bool text)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(8, text ? 10 : 0), window->DC.CursorPos + ImVec2(0, text ? 10 : 0) + ImVec2(window->Size.x - 8, label_size.y + style.FramePadding.y * 2.0f));
    ...
    if (!(flags & ImGuiComboFlags_NoPreview)) {
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(24, 26, 28), 2, popup_open ? ImDrawFlags_::ImDrawFlags_RoundCornersTop : ImDrawFlags_::ImDrawFlags_RoundCornersAll);
    ...
    if (label_size.x > 0 && text)
        RenderText(ImVec2(window->DC.CursorPos.x + 8, window->DC.CursorPos.y - 28), label, 0, true);
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    if (!BeginCombo(label, preview_value, ImGuiComboFlags_None, true))
        return false;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float w = CalcItemWidth();
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(8, 12), window->DC.CursorPos + ImVec2(window->Size.x - 8, 20 + label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    RenderNavHighlight(frame_bb, id);
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10), ImVec2(frame_bb.Max.x, frame_bb.Max.y - 6 - 8), ImColor(60, 60, 60), true, g.Style.FrameRounding);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10), ImVec2(grab_bb.Max.x, grab_bb.Max.y - 3 - 8), ImColor(nigger.x, nigger.y, nigger.z, 255.f), style.GrabRounding);
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(window->DC.CursorPos.x + 8, window->DC.CursorPos.y - 28), label);
    Spacing();
