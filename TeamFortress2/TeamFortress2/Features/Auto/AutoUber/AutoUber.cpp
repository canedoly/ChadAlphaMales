...
{
	if (!Vars::Triggerbot::Uber::Active                    || //Not enabled, return
	...
	if (Vars::Triggerbot::Uber::PopLocal && pLocal->IsVulnerable())
	{
		m_flHealth = static_cast<float>(pLocal->GetHealth());
		m_flMaxHealth = static_cast<float>(pLocal->GetMaxHealth());

		if (((m_flHealth / m_flMaxHealth) * 100.0f) <= Vars::Triggerbot::Uber::HealthLeft)
		{
	...
	if (const auto& pTarget = pWeapon->GetHealingTarget())
	{
		...
		if (Vars::Triggerbot::Uber::OnlyFriends && !g_EntityCache.Friends[pTarget->GetIndex()])
			return;
