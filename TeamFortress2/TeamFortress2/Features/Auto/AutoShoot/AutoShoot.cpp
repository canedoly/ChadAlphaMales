...
bool CAutoShoot::ShouldFire(CBaseEntity* pLocal, CBaseCombatWeapon* pWeapon)
{
	...
	case CLASS_SNIPER:
	{
		if (Vars::Triggerbot::Shoot::WaitForHeadshot.m_Var)
		{
			if (!g_GlobalInfo.m_bWeaponCanHeadShot && pLocal->IsScoped())
				return false;
		}

		if (Vars::Triggerbot::Shoot::ScopedOnly.m_Var)
		{
			const int nWeaponID = pWeapon->GetWeaponID();

			if (nWeaponID != TF_WEAPON_COMPOUND_BOW && pLocal->GetClassNum() == ETFClass::CLASS_SNIPER && pWeapon->GetSlot() == EWeaponSlots::SLOT_PRIMARY)
			{
				if (!pLocal->IsScoped())
					return false;
			}
