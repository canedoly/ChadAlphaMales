...
// You should only use this for indexes, dont actually try to use it
/*class __declspec(novtable) IBaseClientDLL
{
public:
	...
	virtual void			FrameStageNotify(EClientFrameStage curStage) = 0;
	...
	// Save/restore system hooks // THIS IS ALL WRONG LOL I REPLACED ALL THE STUFF WITH VOID *
	/* 
	* ACTUAL
	...
	* virtual void				DispatchOnRestore() = 0;
	...
	virtual void			ReloadFilesInList(/* IFileList*  void *pFilesToReload) = 0;
#ifdef POSIX
	...
	virtual /*CRenamedRecvTableInfo*  void* GetRenamedRecvTableInfos() = 0;

	// Get the mouthinfo for the sound being played inside UI panels
	virtual /*CMouthInfo*  void *GetClientUIMouthInfo() = 0;
